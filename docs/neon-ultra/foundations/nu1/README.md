...
> **Clôture des écarts de vérification (même jour, `fix(nu1)`)** :
> ① vrai 17e test `NeonUltraDesignSystemTests` — Reduce Motion comportemental
> (`NeonUltraMotionResolver`, l'unique chemin de code de pression des trois
> styles de bouton : échelle 1 et animation nil sous réduction, 0,98 animé
> sinon) → **276 tests iOS au total** ; ② cibles tactiles MESURÉES par rendu
> `UIHostingController` (6 primitives ≥ 44 × 44 pt, plus une lecture de
> constantes) ; ③ nuancier complété à **18 rôles** (`textOnCta #FFFFFF`)
> dans les deux galeries + tests ; ④ champ « Intitulé » devenu multiligne
> auto-dimensionné — valeur complète visible à 320 px / 200 %, test de
> troncature INTERNE ajouté (contrôles et champs, pas seulement le
> débordement de page) ; ⑤ un seul CTA primaire actif par section — la
> démonstration de focus est un bouton secondaire sombre, nouvelle capture
> `nu1-pwa-390-focus.png` (Tab réel jusqu'au contrôle).
...
Parité sémantique PWA ↔ iOS verrouillée par test (18 rôles de couleur dont
`textOnCta`, rayons 26/18/14, mouvement 140/240 ms, pression 0,98).
...
|---|---|---|
| `nu1-pwa-390.png` | galerie complète standard | ✓ nuancier 18 rôles (dont text-on-cta), un seul point focal (CTA du héros), aucun glow sur les montants, montant extrême entier |
| `nu1-pwa-320.png` | galerie complète 320 px | ✓ aucun débordement, aucun texte coupé |
| `nu1-pwa-320-texte-200.png` | texte agrandi 200 % à 320 px | ✓ corps de texte doublé, montants fluides ENTIERS, champ « Courses de la semaine » complet (multiligne auto), zéro débordement ni troncature interne |
...
| `nu1-pwa-390-focus.png` | focus clavier réel (Tab) | ✓ anneau CYAN ≥ 2 px avec offset sur le bouton SECONDAIRE, un seul CTA gradient actif dans le viewport |
