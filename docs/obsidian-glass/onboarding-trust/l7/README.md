...
| `l7-390-import-avant-confirmation.png` | Import CSV | contenu collé, « rien n'est écrit avant votre confirmation » | 390×844 | — |
...
- L'onboarding natif n'apparaît pas dans le tour Demo (le mode démo
  démarre app ouverte) : il est couvert par les tests unitaires
  (`ObsidianTrustTests`, `OnboardingViewModelTests`) et les previews.
- La PWA n'a pas de Face ID et ne le prétend jamais.

## Preuve native (iOS)

Tour Demo asserté : `05-plus` (hub par intentions), `10-reglages`,
**`16-documents`** (nouveau) et **`17-suppression-annulee`** (nouveau —
le dialogue de suppression totale est OUVERT, prouvé nommant la portée
et proposant « D'abord créer une sauvegarde », puis ANNULÉ sans toucher
aux données). Artefact `budget-demo` du run référencé dans
`OBSIDIAN_GLASS_STATUS.md`.
