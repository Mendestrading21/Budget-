...
services réels (AccountBalanceService, MonthlySnapshotService,
NetWorthService), comparaison champ par champ — preuve par le job
simulateur CI. Écarts natifs CONSIGNÉS dans `enAttenteNatif` (jamais
un skip silencieux) : conversion FX des agrégats et soldes
multi-devises → W4 (constat n° 6 de l'audit). Gate CI : étape « Runner
canonique Web » ajoutée au job navigateur ; le runner Swift vit dans
le job simulateur. FI-40 a désormais sa gate mécanique.
