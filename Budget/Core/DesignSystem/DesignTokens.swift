...
/// Rôles historiques conservés comme façade de compatibilité.
...
enum BudgetColor {
    // MARK: - Rôles de compatibilité alignés sur Neon Ultra
    ...
    /// `rgba(124,58,237,0.48)` — sélection ou focus discret.
    static let strokeActive = rgb(124, 58, 237, alpha: 0.48)
    /// `#38BDF8` — action et information. Même autorité que Neon Ultra.
    static let brand = rgb(56, 189, 248)
    /// `#38BDF8` — surbrillance accessible de la même autorité.
    static let brandBright = rgb(56, 189, 248)
    ...
    /// `#35D39A` — sémantique : progrès, résultat favorable.
    static let positive = rgb(53, 211, 154)
    /// `#FF6577` — sémantique : perte, dépassement, erreur.
    static let negative = rgb(255, 101, 119)
    /// `#F6C453` — sémantique : échéance, attention.
    static let warning = rgb(246, 196, 83)

    // MARK: - Ton profond du CTA

    /// `#6E00E8` — extrémité sombre du CTA : texte blanc AA (≥ 4.5:1).
    static let brandDeep = rgb(110, 0, 232)
...
/// consulté : l'identité Budget Prisme est unique et sombre.
enum BudgetTheme {
    static func screenBackground(_ scheme: ColorScheme) -> some ShapeStyle {
        _ = scheme
        return AnyShapeStyle(BudgetColor.canvas)
    }
    ...
    static func cardBorder(_ scheme: ColorScheme) -> LinearGradient {
        ...
        return LinearGradient(
            colors: [BudgetColor.stroke, BudgetColor.stroke],
...
extension LinearGradient {
    /// Alias de compatibilité : un seul dégradé CTA dans toute l'app.
    static let budgetAccent = NeonUltraGradient.cta

    /// Rampe de la série principale des graphiques — cyan unique.
...
// ------------------------------------------------------------
// AUTORITÉ VISUELLE : les wrappers historiques ci-dessus convergent vers
// cette palette afin que les écrans migrés et non migrés ne divergent plus.
// Les valeurs brutes vivent ici ; les primitives `NeonUltra*` les utilisent.
...
enum NeonUltraGradient {
    ...
    /// Signature Budget Prisme. Ce liseré n'est utilisé que sur le héros
    /// d'un écran ou une sélection explicite — jamais sur toutes les cartes.
    static let prismEdge = LinearGradient(
        colors: [
            NeonUltraColor.cyan,
            NeonUltraColor.violet,
            NeonUltraColor.magenta,
        ],
        startPoint: .topLeading,
        endPoint: .bottomTrailing
    )
...
/// Mesures communes de l'iconographie Budget Prisme.
enum NeonUltraIconMetric {
    /// Puits compact dans une ligne ou une métrique.
    static let well: CGFloat = 40
    /// Puits interactif, donc cible tactile complète.
    static let controlWell: CGFloat = 44
    /// Glyphe optique à l'intérieur du puits.
    static let glyph: CGFloat = 18
    /// Rayon continu du puits.
