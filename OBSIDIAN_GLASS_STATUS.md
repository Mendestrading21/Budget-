...
| L9 Audit final | VERIFYING | **Passe corrective du 25.07.2026 TERMINÉE** (commits `317bba0` refus/ADR-023 + `9e0a754` fix + `b7e3870` réconciliation) : cible native **iPhone uniquement** prouvée (`TARGETED_DEVICE_FAMILY = 1` × 6, orientations iPad retirées ; CI #218/#219 : `UIDeviceFamily == [1]` dans le produit Release + `TARGETED_DEVICE_FAMILY = 1` en Debug ET Release via showBuildSettings ; Demo #34 : `UIDeviceFamily == [1]` dans l'app de l'ARCHIVE et dans l'app EXTRAITE de l'IPA) ; **`DiskStoreLifecycleTests` créé et VERT en CI** (store disque réel : écrire → détruire le conteneur → rouvrir → relire par UUID, Decimal et relation exacts) ; **charset corrigé** (`<meta charset="utf-8">` première ligne + test 72 servi en HTTP SANS charset) ; réconciliation documentaire complète (21 captures, AppLockManagerTests 4 / BackupServiceTests 10, PERF par source). **CI FINALE #219 verte** (run 30171943844 sur `b7e3870`) : **72 e2e + 5 parités + design, 259 tests iOS 0 échec**, builds Debug+Release, PrivacyInfo + UIDeviceFamily vérifiés dans Budget.app — PERF finale : répartis 23 ms · concentrés 33 ms · navigation 24 ms · recherche 18 ms/111 lignes · défilement 27 ms (200 lignes DOM/page, 10 000 mouvements). **Demo #34 vert** (run 30171837501 sur `9e0a754`, artefact budget-demo 278,2 Mo, expire 23.10.2026). Refus initial documenté sous « Refus L9 n°1 » : cible iPad résiduelle contredisant la fiche, test disque documenté mais inexistant, P2 charset à corriger réellement, écarts documentaires — décomptes de captures/tests et mesures PERF recopiées d'un run antérieur). Historique de la passe initiale conservé ci-dessous : passe d'audit du 25.07.2026 sur HEAD `35c9790` (départ : CI #215 verte, run 30166009397). Preuves dans `docs/obsidian-glass/final-audit/l9/` : matrice écran/bouton PWA+iOS (14 espaces + transverses, preuves A/V/H, PASS partout), invariants financiers chacun rattaché à un test NOMMÉ, audit store disque/migrations/sauvegarde (refus atomiques prouvés), audit navigateur **70/70 contrôles PASS** (5 onglets + 10 destinations + détail + feuille, 390 ET 320, exclusion du ＋ à l'ouverture et après défilement, persistance après rechargement, **service worker actif + rechargement HORS LIGNE réussi** sur https local, installabilité), **21 captures** inspectées une à une (montants 7 chiffres sans troncature), audit confidentialité/App Store (bundle `ch.budgetapp.Budget`, 1.0 (1), iOS 17, PrivacyInfo « aucune donnée », zéro réseau vérifié ; 9 points HUMAN REQUIRED listés), protocole iPhone réel + haptique consigné **PENDING HUMAN**. Suites locales : `git diff --check` OK, 3× `node --check` OK, **71 e2e + 5 parité + design verts, zéro erreur console** (PERF L8 : 32/30 ms pour 10k). Défauts : **P0 : 0 · P1 : 0 · P2 : 1** (PWA sans `<meta charset>` — cassure démontrée sur serveur sans en-tête charset ; canaux réels non affectés ; correctif d'une ligne PROPOSÉ, non appliqué — aucun parcours livré ne régresse) · **P3 : 4** — registre complet dans `DEFECTS.md`. CI finale + Demo du commit documentaire : voir « Preuves finales L9 » ci-dessous | validation humaine des preuves + contrôle haptique physique par le propriétaire (`/budget-v1 verify L9`) |

Statuts autorisés : `BLOCKED`, `READY`, `IN_PROGRESS`, `VERIFYING`, `DONE`.

## Refus L9 n°1 (25.07.2026) et décisions définitives du propriétaire

La validation humaine de la passe d'audit L9 (`2ce7320`) est **REFUSÉE**.
Décisions DÉFINITIVES du propriétaire, 25.07.2026 :

- la V1 native prend en charge **uniquement l'iPhone** — aucune prise en
  charge iPad native n'est demandée (ADR-023) ;
- Budget n'a encore **jamais** été installée sur un iPhone réel ;
- **aucun** TestFlight ni Xcode local n'a encore été utilisé ;
- **aucun** compte Apple Developer / App Store Connect n'est configuré ;
- aucune QA physique ne peut donc être déclarée réussie aujourd'hui.

Défauts de la passe refusée, à corriger dans cette passe corrective :

1. **Cible iPad résiduelle** : `TARGETED_DEVICE_FAMILY = "1,2"` (6
   occurrences) + orientations iPad déclarées, en contradiction avec la
   fiche App Store et la décision produit → passage à `1`, retrait des
   deux réglages d'orientations iPad, contrôles CI exigeant
   `UIDeviceFamily == [1]` (liste ENTIÈRE) dans le produit Release CI,
   l'app de l'archive Demo ET l'app extraite de l'IPA, plus preuve
   secondaire `xcodebuild -showBuildSettings` Debug et Release.
2. **Test de persistance disque inexistant** : l'audit citait le
   lancement Demo comme seule preuve disque → création RÉELLE de
   `DiskStoreLifecycleTests` (URL temporaire unique, configuration de
   production `isStoredInMemoryOnly: false`, données UUID + Decimal +
   relation, save, destruction du premier conteneur, relecture par un
   second conteneur indépendant, vérification exacte, nettoyage).
3. **P2 charset non corrigé** : `<meta charset="utf-8">` ajouté en
   PREMIÈRE ligne de `webapp/index.html` + scénario navigateur dédié
   servi en HTTP SANS charset dans l'en-tête (démarrage réel + texte
   accentué exact + zéro erreur) — suite portée à 72 parcours minimum.
4. **Écarts documentaires** : 21 captures PWA (pas 23), matrice = 14
   espaces + 12 parcours transverses, les 4 tests de verrouillage
   vivent dans `AppLockManagerTests` (PAS BackupServiceTests, qui en
   contient 10), `testRestoreRefusesNonCHFAccounts` appartient à
   `UnifiedTaxReserveTests`, mesures PERF recopiées d'un run antérieur
   au lieu du run cité (valeurs exactes de CI #217 : répartis 24 ms ·
   concentrés 34 ms · navigation 23 ms · recherche 15 ms/111 lignes ·
   défilement 29 ms) → réconciliation complète et distinction
   local / CI précédente / CI finale.
...
## Preuves finales L9 — passe corrective (vérifiées le 25.07.2026)

- Commits : `317bba0` (`docs(l9): record corrective review and
  iPhone-only decision`) + `9e0a754` (`fix(l9): close final release
  readiness gaps`) + `b7e3870` (`docs(l9): reconcile corrective
  evidence`).
- **CI #218 verte** (run 30171833100, sur `9e0a754`) : 72 e2e + 5
  parités + design ; **259 tests iOS, 0 échec** (dont
  `DiskStoreLifecycleTests.testDataWrittenOnDiskSurvivesFullContainerTeardown`
  passé) ; builds Debug + Release ; PrivacyInfo dans Budget.app ;
  « UIDeviceFamily == [1] dans le produit Release ✓ » ;
  « TARGETED_DEVICE_FAMILY = 1 en Debug ✓ / en Release ✓ ».
- **Demo #34 vert** (run 30171837501, sur `9e0a754`) : tours assertés,
  étape « iPhone uniquement — UIDeviceFamily == [1] (archive + IPA) »
  VERTE (app de l'archive ET app extraite de l'IPA non signée),
  artefact **budget-demo 278,2 Mo** (captures + vidéo + IPA), expire le
  23.10.2026 :
  <https://github.com/Mendestrading21/Budget-/actions/runs/30171837501>.
- **CI FINALE #219 verte** (run 30171943844, sur `b7e3870`) : mêmes
  totaux (72/5/259/0) ; **PERF de référence FINALE** : répartis
  **23 ms** · concentrés **33 ms** · navigation **24 ms** · recherche
  **18 ms** (111 lignes) · défilement **27 ms** — 200 lignes DOM par
  page pour 10 000 mouvements.
- Bases minimales désormais : **72 e2e / 5 parités / 259 tests iOS /
  0 échec / 0 erreur console / UIDeviceFamily == [1]** — jamais moins.

## Preuves finales L9 — passe initiale (25.07.2026, historique)
...
```text
(aucune : L9 = VERIFYING — la suite appartient au propriétaire)
```

L1 à L8 sont DONE. **L9 est VERIFYING** : la passe corrective est
terminée et prouvée (voir « Preuves finales L9 — passe corrective »).
`/budget-v1 verify L9` ne devra être lancé qu'APRÈS : (1) création du
compte Apple Developer et installation par TestFlight sur l'iPhone
réel — l'app n'a encore JAMAIS été installée —, (2) confirmation
humaine du HAPTIQUE (4 gestes de la checklist), de Face ID réel et de
VoiceOver réel (`docs/obsidian-glass/final-audit/l9/IPHONE_QA_CHECKLIST.md`).
Claude ne peut ressentir ni vibration ni Face ID : L9 ne sera jamais
marqué DONE sans ces confirmations du propriétaire.
